...
namespace ConsoleApp1
{
    internal static class Program
    {
        private static void Main()
        {
            ...
            Book[] arr = { b1, b2, b3 };
            arr[0] = new Book(1, "Clean Code (Updated)", 2008, 520m, 10);
            int idx = Array.FindIndex(arr, b => b.SerialNumber == 3);
            Array.Resize(ref arr, arr.Length + 1);
            arr[^1] = b4;
            Console.WriteLine($"Array (found SN=3 at index {idx}):");
            ...
            list.Add(b3);
            list.Remove(b2);
            var f = list.Find(b => b.SerialNumber == 3);
            if (f is not null) f.IncreasePriceByPercent(10);
            ...
            al.Add(b3);
            al.Remove(b2);
            al[0] = new Book(1, "Clean Code (AL)", 2008, 515m, 10);
            Book? found = null;
            foreach (var it in al) if (it is Book bk && bk.SerialNumber == 3) { found = bk; break; }
            Console.WriteLine($"ArrayList (found SN=3: {found is not null}):");
